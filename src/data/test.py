...
df["num_bed_rooms"] = (
    df["num_bed_rooms"].str.replace(" phòng", "").str.strip().astype(float)
)
df["squared_meter_area"] = (
    df["squared_meter_area"].str.replace(" m²", "").str.strip().astype(float)
)
...
df_cleaned = df_cleaned.drop(
    ["index", "date", "address", "district", "ward", "type_of_housing", "legal_paper"],
    axis=1,
)
df_cleaned.head()
...
print(f"Accuracy for the LR model is: {accuracy:.2f}%")
